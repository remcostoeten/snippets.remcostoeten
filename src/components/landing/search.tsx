...
export function Search({
  ...
  const highlightMatch = (text: string, query: string) => {
    ...
    try {
      ...
      return text.replace(
        regex,
        '<mark class="bg-purple-500/20 text-purple-200 rounded px-0.5">$1</mark>'
      );
    } catch (e) {
      return text;
    }
  ...
  const dropdownPosition =
    variant === "header" ? "top-[calc(100%+8px)]" : "top-full";

  const groupedResults =
    query.data && query.data !== "empty"
      ? groupResults(query.data as SearchResult[])
      : [];

		...
		return {
			...
			deployment: {
				...
				timestamp: deploymentTime
					? new Date(deploymentTime).getTime()
					: null
			}
		}
	} catch (error) {
...
export async function DeploymentInfo() {
	...
	return (
		<div className='text-sm text-gray-500'>
			<p>
				Last commit: {formatTimeAgo(info.commit.timestamp * 1000)} (
				{info.commit.count} commits)
			</p>
			{info.deployment.timestamp && (
				<p>
					Last deployment: {formatTimeAgo(info.deployment.timestamp)}
				</p>
			)}
		</div>
	)

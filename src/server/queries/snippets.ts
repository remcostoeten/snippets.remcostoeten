...
const LANGUAGE_PATTERNS: Record<string, { identifiers: string[] }> = {
    ...
    Databases: {
        identifiers: ['sql', 'sqlite', 'psql', 'postgres', 'postgresql', 'drizzle', 'prisma', 'turso']
    },
    Git: {
        identifiers: ['git', 'gitignore', 'git-set-upstream', 'git-branch-diverged']
    },
    Blog: {
        identifiers: [
            'blog',
...
const LANGUAGE_COLORS: Record<string, string> = {
    ...
    Databases: 'bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20',
    JSON: 'bg-sky-500/10 text-sky-400 border-sky-500/20 hover:bg-sky-500/20',
    Shell: 'bg-green-500/10 text-green-400 border-green-500/20 hover:bg-green-500/20',
    TypeScript: 'bg-purple-500/10 text-purple-400 border-purple-500/20 hover:bg-purple-500/20',
    ...
    Unknown: 'bg-zinc-500/10 text-zinc-400 border-zinc-500/20 hover:bg-zinc-500/20'
}
        ...
        for (const file of mdxFiles) {
            ...
            if (frontmatter.category) {
                ...
                categoryMap.set(category, {
                    ...
                    description: current.description || frontmatter.categoryDescription || ''
                })
            }
        }
    ...
    try {
        ...
        const snippets = await Promise.all(
            mdxFiles.map(async (file) => {
                ...
                return {
                    title: frontmatter.title || file.split('/').pop()?.replace('.mdx', ''),
        ...
        const validSnippets = snippets.filter((snippet): snippet is NonNullable<typeof snippet> => snippet !== null)

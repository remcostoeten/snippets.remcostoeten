...
const getGitInfo = cache(async () => {
    ...
    try {
        ...
        const { stdout: v2CommitCount } = await execAsync(`git rev-list --count HEAD ^${V2_START_COMMIT}`)

        // Get total number of commits for v1
        const { stdout: v1CommitCount } = await execAsync(`git rev-list --count ${V2_START_COMMIT}^`)

        // Get latest commit info
        const { stdout: lastCommitInfo } = await execAsync('git log -1 --pretty=format:"%H|%ct|%s"')
        const [hash, timestamp, message] = lastCommitInfo.split('|')

        // Get total number of posts (MDX files)
        const { stdout: postCount } = await execAsync('find content/docs -type f -name "*.mdx" | wc -l')
        ...
        const totalDeployments =
            process.env.VERCEL_ENV === 'production' ? Number.parseInt(process.env.VERCEL_DEPLOYMENT_COUNT || '1') : 1

        const info = {
            totalPosts: Number.parseInt(postCount.trim()),
            totalCommits: {
                v1: Number.parseInt(v1CommitCount.trim()),
                v2: Number.parseInt(v2CommitCount.trim()),
                total: Number.parseInt(v1CommitCount.trim()) + Number.parseInt(v2CommitCount.trim())
            },
            totalDeployments,
            lastCommit: {
                hash,

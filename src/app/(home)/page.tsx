...
function formatDescription(description: string) {
    return description.split(/(`[^`]+`)/).map((part, index) => {
        if (part.startsWith('`') && part.endsWith('`')) {
            return (
                <code key={index} className="px-1.5 py-0.5 rounded bg-zinc-800 font-mono text-xs text-zinc-300">
                    {part.slice(1, -1)}
                </code>
            )
        }
                ...
                <section className="container py-12">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex items-center gap-4">
                            {remainingCount > 0 && (
                                <TextAnimate as="span" animation="fadeIn" delay={0.7} className="text-sm text-zinc-400">
                                    <AnimatedNumber value={remainingCount} suffix=" more snippets" />
                                </TextAnimate>
                            )}
                            <TextAnimate as="span" animation="fadeIn" delay={0.8} className="text-zinc-400">
                                /
                            </TextAnimate>
                            <Link
                                href="/docs"
                    ...
                    <div className="grid gap-4">
                        {snippets.map((snippet, index) => (
                            <TextAnimate key={snippet.title} as="div" animation="fadeIn" delay={0.6 + 0.1 * index}>
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div className="flex-grow">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    >
                                                        <GradientText variant="subtle">{snippet.title}</GradientText>
                                                    </TextAnimate>
                                                ...
                                                >
                                                    <span>{new Date(snippet.lastModified).toLocaleDateString()}</span>
                                                    <span>•</span>
                                                    <AnimatedNumber value={snippet.metrics.wordCount} suffix=" words" />
                                                    <span>•</span>
                                                    <AnimatedNumber

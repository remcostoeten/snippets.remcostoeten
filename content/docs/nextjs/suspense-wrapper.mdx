...
<Warning>
	Suspense wrappers are not a magic fix. If used improperly, they can harm UX
	more than help.
</Warning>
...
<p className='text-gray-400'>
	A Suspense Wrapper is a composable React component built on top of React’s
	native `<Suspense />
	`. It lets you render a fallback UI (like a skeleton or spinner) while
	waiting for a nested async component to load or resolve its data.
</p>

<Important>
	In Next.js 14+ with SSR, Suspense can render loading states during server
	rendering — great for skeleton UIs and smoother hydration.
</Important>

## ✅ When to Use It

<Tip>Use Suspense Wrappers when:</Tip>
...
<Warning>Avoid Suspense Wrappers:</Warning>
...
<Example>
	Encapsulates loading logic, avoids layout shift, and enhances UX
	consistency.
</Example>
...
<Warning>
	This has no real effect. Client components aren’t blocked by Suspense, so
	fallback won’t be shown.
</Warning>
...
<p className='text-gray-400'>
	You can nest wrappers to load independent parts of the page asynchronously —
	for example, a sidebar, main content, and notifications section.
</p>
...
<Tip>Follow these for optimal results:</Tip>
...
<Warning>Common mistakes include:</Warning>
...
<p className='text-gray-400'>
	Suspense Wrappers are a powerful tool when paired with SSR and `use()` in
	Next.js 14+. When used correctly, they deliver fluid loading experiences
	with minimal effort and clear separation of concerns.
</p>

<Tip>
	Think of Suspense as layout-preserving progressive rendering. Skeletons
	serve as visual contracts.
</Tip>

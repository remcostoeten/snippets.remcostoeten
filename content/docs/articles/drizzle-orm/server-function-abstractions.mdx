...
// Simplified type definitions to avoid MDX parsing issues
type SchemaRow<T> = any
type SchemaColumn<T> = string

export function CrudAbstractions(schema: any) {
  type Row = any
  type Column = string
...
Try out the CRUD abstractions with our interactive playground! Build queries visually and see the generated code in real-time.

<div className="my-8 p-4 bg-zinc-900/50 rounded-lg border border-zinc-800">
  <div className="flex items-center gap-2 mb-4">
    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
    <span className="text-sm text-zinc-400">Live Playground</span>
  </div>
  <p className="text-sm text-zinc-300 mb-4">
    Paste your Drizzle schema, build queries with the visual builder, and generate production-ready CRUD code.
  </p>
  <a 
    href="/query-builder" 
...
- **Schema Parser**: Paste your Drizzle schema and see it visualized
- **Visual Query Builder**: Build CRUD operations with a drag-and-drop interface  
- **Code Generator**: Get production-ready TypeScript code instantly
- **Real-time Preview**: See your queries execute against a mock database

### Example Workflow

1. **Parse Schema**: Paste your Drizzle table definitions
2. **Browse Tables**: Explore your schema structure and relationships
3. **Build Queries**: Use the visual builder to create CRUD operations
4. **Generate Code**: Copy the generated TypeScript code for your project
